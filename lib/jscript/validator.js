...
Validator.add('firstName','string',{required: 1, maxLength: 10, minLength: 3}); 	//firstName is required, and must be between 3 and 10 chars.
Validator.add('emailAddress','string',{required: 1, maxLength: 15, minLength: 5});//emailAddress is required, and must be between 5 and 15 chars.  
	...
	*/
	this.add = function(id,type, options){
		...
		if(type=='string'){
			...
			element.addValidator(new ValidatorPhoneValidator(element, options));
		}
		else if(type=='regex'){
			element.addValidator(new ValidatorRegexValidator(element, options));
		}
		else{
...
function ValidatorElement(id){
	...
	this.failure = function(defaultMessage, message){
		if(typeof(message) == "undefined"){
			message=defaultMessage;
		}
		
		
		if(this.validates==0 && this.failValue == this.getValue())return;
		this.highlight();
		if(message!=""){
			alert(message);
		}
		this.validates=0;
		this.failValue = this.getValue();
	}

	this.highlight = function(){
		document.getElementById(this.id).style.background=Validator.hilight;
	}

	this.unHighlight = function(){
		document.getElementById(this.id).style.background=Validator.dehilight;
	}
	...
	this.validate = function(){
		...
		if(this.options.minLength){
			if(this.options.minLength > elementValue.length && elementValue.length !=0){
				this.element.failure("This field must have at least " + this.options.minLength + " characters. It currently has " + elementValue.length + ".", this.options.message);
				return false;
			}
		}
		return 1;
	...
	this.validate = function(){
		...
		for (i = 0; i < elementValue.length && isNumber == true; i++)
		{
			...
			{	
				...
				this.element.failure(failText, this.options.message);
				return false;
			}
		}
		return true;
...
function ValidatorEmailValidator(element,options){
	...
	this.validate = function(){
		...
		if (!((elementValue.indexOf(".") > 0) && (elementValue.indexOf("@") > 0))){
			this.element.failure("This field must only contain email addresses. Please entar a valid email address.",this.options.message);
			return false;
		}
		return true;
	}
...
* options:	regex(regular expression) - the regular expression to test against
*/
function ValidatorRegexValidator(element,options){
	this.options=options;
	this.element=element;
	...
	this.validate = function(){
		...
		//don't continue if the value is empty
		if(elementValue=='')return true;
		if(!this.options.regex){
			return true;
		}
		if(this.options.regex.test(elementValue)){
			return true;
		}else{
			this.element.failure("This field does not Validate. Please correct it.", this.options.message);
			return false;
		}
...
/**
* Simple validator for phone numbers.
*
* Actually just a decorator for ValidatorRegexValidator.
* 
* options:	none.
*/
function ValidatorPhoneValidator(element,options){
	options.regex=/^(\()?[0-9]{3}(\))?-?(\s)?([0-9]){3}(-?)([0-9]){4}$/;
	if(typeof(options.message)=='undefined'){
		options.message="This field must only contain valid phone numbers";
	}
	this.validator=new ValidatorRegexValidator(element,options);
	//Validate function. This is called by a ValidatorElement
	this.validate = function(){
